...
class simulation:
    ...
    def generate_mesh(self):
        ...
        for boundary in boundaries:
            ...
            elif np.allclose(center_of_mass, [self.L / 2, self.H, 0]) or np.allclose(center_of_mass,
                                                                                     [self.L / 2, 0, 0]):
    ...
    def compute(self, mesh, ft, points, jet, save='False'):
        ...
        if mesh.comm.rank == 0:
            p_diff = np.zeros(self.num_steps, dtype=PETSc.ScalarType)
        ...
        drag_coeffs = []
        lift_coeffs = []
        progress = tqdm.autonotebook.tqdm(desc="Solving PDE", total=self.num_steps)
        for i in range(self.num_steps):
            ...
            drags = assemble_scalar(drag)
            lifts = assemble_scalar(lift)
        ...
        return t_u, t_p, p_diff, C_D, C_L, num_velocity_dofs, num_pressure_dofs
