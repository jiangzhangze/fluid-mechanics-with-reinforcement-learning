...
base_config = {
    ...
    "observation_space": gym.spaces.Box(low=-2, high=2, shape=([100 * 2 * 7 * 1600, ]), dtype=np.float16)
...
class BaseEnv(gym.Env):
    def __init__(self, config=None):
        ...
        self.action_space = config.get("action_space", gym.spaces.Box(low=np.array([0, 0]), high=np.array([2.2, 0.41]),
                                                                      dtype=np.float16))
        # we take the value of the velocity probes as the observation.
        self.observation_space = config.get("observation_space",
                                            gym.spaces.Box(low=-2, high=2, shape=([self.num * 2 * 7 * 1600, ]),
                                                           dtype=np.float16))
